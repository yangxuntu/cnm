...
from .CaptionModel_rs4 import CaptionModel
...
class AttModel_rs3(CaptionModel):
    def __init__(self, opt):
        super(AttModel_rs3, self).__init__()
        ...
        self.sg_net_index = getattr(opt, 'sg_net_index', 1)
        ...
        if self.sg_net_index:
            self.sg_net = SG_net(opt)
    ...
    def _sample_beam(self, rs_data, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state, _ = self.get_logprobs_state(it, tmp_rs_data, state)

            self.done_beams[k] = self.beam_search(state, logprobs, tmp_rs_data, opt=opt)
...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.gum_soft = getattr(opt, 'gum_soft', 0)
        if self.combine_att == 'concat':
            self.lang_lstm = nn.LSTMCell(opt.rnn_size * 5, opt.rnn_size)
        ...
        elif self.cont_ver == 2:
            self.controller = Mod_Cont(opt)
            # 4 means the number of module networks, if more modules are used, this number should be changed
            self.rnn_size = opt.rnn_size
            self.att_hid_size = opt.att_hid_size
            self.W_cont = nn.Linear(opt.rnn_size, opt.rnn_size)
            self.c2a = nn.Linear(opt.rnn_size, opt.att_hid_size)
            self.h2a = nn.Linear(opt.rnn_size, opt.att_hid_size)
            self.a_net = nn.Linear(opt.att_hid_size, 1)
        elif self.cont_ver == 3:
            self.controller = Mod_Cont(opt)
            # 4 means the number of module networks, if more modules are used, this number should be changed
            self.W_cont = nn.Linear(opt.rnn_size, 4)
    ...
    def forward(self, xt, rs_data, state):
        ...
        if self.cont_ver == 1:
            h_new, c_new = self.controller(att_lstm_input, state[0][2], state[1][2])
            if self.gum_soft == 1:
                cont_weights = F.gumbel_softmax(self.W_cont(h_new), hard=True)
            else:
            ...
            h_new, c_new = self.controller(att_lstm_input, state[0][2], state[1][2]) # batch_size*rnn_size
            att_all = torch.stack((att_lang, att_obj, att_rela, att_attr), dim=1)  #batch_size*4*rnn_size
            att_p = self.c2a(att_all)  #batch_size*4*att_hide_size
            att_h = self.h2a(h_new)  # batch * att_hid_size
            att_h = att_h.unsqueeze(1).expand_as(att_p)  # batch * 4 * att_hid_size
            dot = att_p + att_h  # batch * 4 * att_hid_size
            dot = F.tanh(dot)  # batch * 4 * att_hid_size
            dot = dot.view(-1, self.att_hid_size)  # (batch * 4) * att_hid_size
            dot = self.a_net(dot)  # (batch * 4) * 1
            dot = dot.view(-1, 4)  # batch * 4
            cont_weights = F.softmax(dot, dim=1)  # batch * att_size
            cw_logit = F.log_softmax(dot, dim=1)
            ...
            cont_input = torch.cat([att_lstm_input, att_obj, att_obj, att_attr], 1)
            h_new, c_new = self.controller(cont_input, state[0][2], state[1][2])
            if self.gum_soft == 1:
                cont_weights = F.gumbel_softmax(self.W_cont(h_new),dim=1)
            else:
                cont_weights = F.softmax(self.W_cont(h_new),dim=1)
            cw_logit = F.log_softmax(self.W_cont(h_new), dim=1)
            att_obj = att_obj * cont_weights[:, 1].unsqueeze(1).expand_as(att_obj)
            ...
            cw_logit = rs_data['obj_feats_new'].new_ones(rs_data['obj_feats_new'].size(0), 4)
        ...
        if self.cont_ver != 0:
            state = (torch.stack([h_att, h_lang, h_new]), torch.stack([c_att, c_lang, c_new]))
        else:
...
class MTopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        #cont_ver=0:  only att_lstm and lang_lstm
        #cont_ver!=0: att_lstm, lang_lstm, and, cont_lstm
        if self.cont_ver == 0:
            self.si = 2
        else:
            self.si = 3
    ...
    def forward(self, xt, rs_data, state):
        for i in range(self.mtopdown_num):
            _, (state[0][self.si * i:self.si * (i + 1)], state[1][self.si * i:self.si * (i + 1)]), cw_logit = \
                self.topdown_layers[i](xt, rs_data,
                                       (state[0][self.si * i:self.si * (i + 1)].clone(),
                                        state[1][self.si * i:self.si * (i + 1)].clone()))
            if self.mtopdown_res:
                xt = xt + state[0][self.si*i+1].clone()
            else:
                xt = state[0][self.si*i+1].clone()
...
class Cap_Reason3(AttModel_rs3):
    def __init__(self, opt):
        super(Cap_Reason3, self).__init__(opt)
        if opt.cont_ver!=0:
            self.num_layers = 3
        else:
            self.num_layers = 2
...
class MCap_Reason3(AttModel_rs3):
    def __init__(self, opt):
        super(MCap_Reason3, self).__init__(opt)
        if opt.cont_ver!=0:
            self.num_layers = 3*opt.mtopdown_num
        else:
            self.num_layers = 2*opt.mtopdown_num
        self.core = MTopDownCore(opt)

#SG_net here do not contain any relationship embedding
class SG_net(nn.Module):
    def __init__(self, opt):
        super(SG_net, self).__init__()
        self.obj_num = getattr(opt, 'obj_num', 100)
        self.attr_num = getattr(opt, 'attr_num', 100)

        self.att_feat_size = opt.att_feat_size
        self.drop_prob_lm = opt.drop_prob_lm

        self.sg_pt = getattr(opt, 'sg_pt', 0)
        self.sg_ft = getattr(opt, 'sg_ft', 1)

        self.obj_fc = nn.Sequential(nn.Linear(self.att_feat_size, self.att_feat_size),
                                    nn.ReLU(inplace=True),
                                    nn.Dropout(self.drop_prob_lm))
        self.attr_fc = nn.Sequential(nn.Linear(self.att_feat_size, self.att_feat_size),
                                    nn.ReLU(inplace=True),
                                    nn.Dropout(self.drop_prob_lm))

        self.obj_cls = nn.Linear(self.att_feat_size, self.obj_num)
        self.attr_cls = nn.Linear(self.att_feat_size, self.attr_num)

    def forward(self, rs_data):
        if self.sg_pt == 1:
            rs_data['obj_fc1'] = self.obj_fc(rs_data['obj_feats'])
            rs_data['attr_fc1'] = self.attr_fc(rs_data['attr_feats'])

            rs_data['obj_cls'] = self.obj_cls(rs_data['obj_fc1'])
            rs_data['attr_cls'] = self.attr_cls(rs_data['attr_fc1'])

            rs_data['obj_logits'] = F.log_softmax(rs_data['obj_cls'], dim = 2)
            rs_data['rela_logits'] = F.log_softmax(rs_data['rela_cls'], dim=2)
        else:
            rs_data['obj_fc1'] = self.obj_fc(rs_data['att_feats'])
            rs_data['attr_fc1'] = self.attr_fc(rs_data['attr_feats'])

            if self.sg_ft == 0:
                rs_data['obj_fc1'] = rs_data['obj_fc1'].detach()
                rs_data['attr_fc1'] = rs_data['attr_fc1'].detach()
...
class Mod_Cont(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.drop_prob_lm = opt.drop_prob_lm
        if opt.cont_ver == 1:
            self.con_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size)
        elif opt.cont_ver == 3:
            self.con_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 5, opt.rnn_size)

    def forward(self, input, cont_h, cont_c):
        h, c = self.con_lstm(input, (cont_h, cont_c))
        return h, c
