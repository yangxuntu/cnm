...
def encode_captions(imgs, params, wtoi):
  ...
  for i,img in enumerate(imgs):
    ...
    Lm = np.zeros((n, max_length), dtype='uint32')
    for j,s in enumerate(img['final_captions']):
      ...
      for k,t in enumerate(tags):
          if k < max_length:
              if t[1] in module_label.keys():
                  Lm[j,k] = module_label[t[1]]
              else:
                  Lm[j, k] = 0
              module_nums[Lm[j, k]] += 1

    # note: word indices are 1-indexed, and captions are padded with zeros
    label_arrays.append(Li)
    tag_arrays.append(Lm)
  ...
  T = np.concatenate(tag_arrays, axis=0)
...
def main(params):
  ...
  # T is the module tags
  L, T, label_start_ix, label_end_ix, label_length = encode_captions(imgs, params, wtoi)

  # create output h5 file
  N = len(imgs)
  f_lb = h5py.File(params['output_h5']+'_label.h5', "w")
  f_lb.create_dataset("labels", dtype='uint32', data=L)
  f_lb.create_dataset("tags", dtype='int32', data=T) #add module tags
